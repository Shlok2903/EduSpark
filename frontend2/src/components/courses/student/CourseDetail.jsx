...
const CourseDetail = () => {
  ...
  const handleModuleClick = (module) => {
    ...
    if (course.isEnrolled || course.isCreator || course.isAdmin || course.isTutor) {
      ...
      if (window.confirm('You need to enroll in this course to view its content. Would you like to enroll now?')) {
        handleEnroll();
      }
    }
  };
  ...
  const isModuleCompleted = (moduleId) => {
    ...
    for (const section of enrollmentData.sectionProgress) {
      for (const moduleProgress of section.moduleProgress) {
        if (moduleProgress.moduleId.toString() === moduleId.toString() && moduleProgress.isCompleted) {
          return true;
        }
      }
    }
  ...
  const markModuleAsCompleted = async (moduleId) => {
    try {
      const response = await enrollmentService.completeModule(courseId, moduleId);
      if (response.success) {
  ...
  const handleQuizSubmit = async () => {
    ...
    try {
      ...
      if (!selectedModule.content || !selectedModule.content.quiz || !selectedModule.content.quiz.questions) {
      ...
      questions.forEach((question, index) => {
        ...
        if (userAnswerIndex !== undefined && question.options[userAnswerIndex].isCorrect) {
          correctAnswers++;
        }
      });
      ...
      await enrollmentService.submitQuiz(selectedModule.id || selectedModule._id, quizAnswers, score);
    ...
    return (
      <Box mt={4}>
        <Paper className="module-content-container">
          <Box className="module-content-header" p={2} display="flex" justifyContent="space-between" alignItems="center">
          ...
          <Box className="module-content-body" p={3}>
            {/* Video Content */}
            {selectedModule.contentType === 'video' && (
              selectedModule.content && selectedModule.content.videoUrl ? (
                ...
                <Box display="flex" flexDirection="column" alignItems="center" p={4}>
                  <WarningIcon color="warning" style={{ fontSize: 48, marginBottom: 16 }} />
                  <Typography variant="h6" color="textSecondary">Video content is not available.</Typography>
                </Box>
              )
            )}
            ...
            {selectedModule.contentType === 'text' && (
              selectedModule.content && selectedModule.content.text ? (
                <Box className="text-content">
                  <div dangerouslySetInnerHTML={{ __html: selectedModule.content.text }} />
                </Box>
              ) : (
                <Box display="flex" flexDirection="column" alignItems="center" p={4}>
                  <WarningIcon color="warning" style={{ fontSize: 48, marginBottom: 16 }} />
                  <Typography variant="h6" color="textSecondary">Text content is not available.</Typography>
                </Box>
              )
            )}
            ...
            {selectedModule.contentType === 'quiz' && (
              selectedModule.content && selectedModule.content.quiz && selectedModule.content.quiz.questions ? (
                <Box className="quiz-container">
                  {!quizSubmitted ? (
                    <>
                      {selectedModule.content.quiz.questions.map((question, questionIndex) => (
                        <Box key={questionIndex} className="quiz-question" mb={4}>
                          <Typography variant="h6">Question {questionIndex + 1}: {question.question}</Typography>
                          <List>
                            {question.options.map((option, optionIndex) => (
                              <ListItem
                                ...
                                className={`quiz-option ${quizAnswers[questionIndex] === optionIndex ? 'selected' : ''}`}
                              >
                      ...
                      <Box mt={2} display="flex" justifyContent="flex-end">
                        <Button
                          ...
                          color="primary"
                          variant="contained"
                          disabled={Object.keys(quizAnswers).length !== selectedModule.content.quiz.questions.length}
                        >
                          Submit Quiz
                        </Button>
                      </Box>
                    ...
                    <Box className="quiz-results">
                      ...
                      <Typography variant="body1" mb={3}>
                        {quizScore >= 70 ? 'Congratulations! You passed the quiz.' : 'You need 70% to pass this quiz. Try again.'}
                      </Typography>
                ...
                <Box display="flex" flexDirection="column" alignItems="center" p={4}>
                  <WarningIcon color="warning" style={{ fontSize: 48, marginBottom: 16 }} />
                  <Typography variant="h6" color="textSecondary">Quiz content is not available.</Typography>
                </Box>
              )
            )}
          </Box>
          ...
          {selectedModule.contentType !== 'quiz' && (
            <Box className="module-content-footer" p={2} display="flex" justifyContent="flex-end">
              <Button
                onClick={() => {
                  if (!isModuleCompleted(selectedModule.id || selectedModule._id)) {
                    markModuleAsCompleted(selectedModule.id || selectedModule._id);
                  }
                }}
                color="primary"
                variant="contained"
                disabled={isModuleCompleted(selectedModule.id || selectedModule._id)}
              >
                {isModuleCompleted(selectedModule.id || selectedModule._id) ? 'Already Completed' : 'Mark as Completed'}
              </Button>
            </Box>
          )}
        </Paper>
      ...
      <Box className="course-header">
        ...
        <Box className="course-info">
          ...
          <Typography variant="body1" className="course-instructor">
            Instructor: {course.instructor || (course.createdBy && course.createdBy.name) || 'Unknown'}
          </Typography>
        </Box>
      </Box>
      
      {/* Course Content */}
      <Grid container spacing={3} className="course-content">
        {/* Left side - Course description and progress */}
        <Grid item xs={12} md={4}>
          <Card className="course-sidebar">
            <CardContent>
              ...
              {course.isEnrolled && (
                <Box className="progress-container">
                  ...
                  <Box display="flex" justifyContent="space-between">
                    ...
                    <Typography variant="body2">{getCourseProgress()}%</Typography>
                  </Box>
                  <LinearProgress 
                    variant="determinate" 
              ...
              {!course.isEnrolled && !course.isCreator && !course.isAdmin && !course.isTutor && (
                <Button
                  variant="contained"
                  color="primary"
                  fullWidth
        ...
        {/* Right side - Course sections and modules */}
                      ...
                      {section.modules && section.modules.length > 0 ? (
                        section.modules.map((module, moduleIndex) => (
                          <ListItem
                            ...
                            button
                            selected={selectedModule && (selectedModule.id === module.id || selectedModule._id === module._id)}
                            onClick={() => handleModuleClick(module)}
                            className={`module-item ${!course.isEnrolled && !course.isCreator && !course.isAdmin && !course.isTutor ? 'locked' : 'clickable'}`}
                          >
                            <ListItemIcon>
                        ...
                        <Typography variant="body2">No modules found in this section</Typography>
                      )}
                    </List>
                  </AccordionDetails>
                </Accordion>
