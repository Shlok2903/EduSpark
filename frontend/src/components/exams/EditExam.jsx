...
function EditExam() {
    ...
    const loadExam = async () => {
      ...
      try {
        ...
        if (examData) {
          ...
          examData.endTime = new Date(examData.endTime);
          
          setExam(examData);
          setExamHasAttempts(examData.hasAttempts || false);
          
          // Also load the course details
          if (examData.courseId) {
            const courseData = await courseService.getCourseById(examData.courseId);
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      const response = await examService.updateExam(examId, exam);
      ...
      handleError('Failed to update exam. Please try again.');
    } finally {
      setSaving(false);
    }
  };
      ...
      <Box component="form" onSubmit={handleSubmit} sx={{ p: 3, flexGrow: 1, overflowY: 'auto' }}>
        <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
          <Box display="flex" alignItems="center">
            <IconButton onClick={() => navigate(`/courses/${exam.courseId}`)} sx={{ mr: 1 }}>
              <ArrowBackIcon />
            </IconButton>
        ...
        {examHasAttempts && (
          <Alert severity="warning" sx={{ mb: 3 }}>
            This exam already has student attempts. Modifying questions or marks may affect existing results.
          </Alert>
        )}
                                  ...
                                  >
                                    {section.questions.map((question, questionIndex) => (
                                      <Draggable
                                        ...
                                        index={questionIndex}
                                        isDragDisabled={examHasAttempts}
                                      >
                                        {(provided) => (
                                          <Card 
                                            ref={provided.innerRef}
                                                  ...
                                                  {question.options.map((option, optionIndex) => (
                                                    ...
                                                    >
                                                      ...
                                                      <IconButton 
                                                        ...
                                                        sx={{ ml: 1 }}
                                                        disabled={examHasAttempts || option.options?.length <= 2}
                                                      >
                                                        <DeleteIcon />
                                                      </IconButton>
                                                    </Box>
